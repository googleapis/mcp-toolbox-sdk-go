...
// WithSecureParams binds a map of secure parameters.
func WithSecureParams(params map[string]any) ToolOption {
	return func(c *ToolConfig) error {
		for k, v := range params {
			if _, exists := c.SecureParams[k]; exists {
				return fmt.Errorf("duplicate secure parameter binding: parameter '%s' is already set", k)
			}
			if _, exists := c.BoundParams[k]; exists {
				return fmt.Errorf("duplicate parameter binding: parameter '%s' is already set as a regular bound parameter", k)
			}
			c.SecureParams[k] = v
		}
		return nil
