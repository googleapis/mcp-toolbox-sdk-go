...
func (tc *ToolboxClient) newToolboxTool(
	...
	allSecureParamSchemas := make(map[string]ParameterSchema)
	...
	for _, sp := range schema.SecureParameters {
		...
		allSecureParamSchemas[sp.Name] = sp
	...
	if isStrict {
		for boundName := range finalConfig.BoundParams {
			if _, isSec := secSchemaMap[boundName]; isSec {
				return nil, nil, nil, nil, fmt.Errorf("parameter %q is a secure parameter; use BindSecureParam/BindSecureParams instead", boundName)
			}
		...
		for secName := range finalConfig.SecureParams {
			if _, isReg := allParamSchemas[secName]; isReg {
				return nil, nil, nil, nil, fmt.Errorf("parameter %q is a regular parameter; use BindParam/BindParams instead", secName)
			}
			if _, exists := secSchemaMap[secName]; !exists {
				return nil, nil, nil, nil, fmt.Errorf("unable to bind secure parameters: no secure parameter named %q found on tool %q", secName, name)
			}
		}
	}
	...
	tt := &ToolboxTool{
		...
		allParamSchemas:         allParamSchemas,
		secureParameters:        finalSecureParameters,
		allSecureParamSchemas:   allSecureParamSchemas,
		transport:               tr,
