...
func TestLoadTool_WithSecureParams(t *testing.T) {
	mcpTools := []mcpTool{
		{
			Name:        "secure-tool",
	...
	t.Run("Successfully loads and binds secure parameters", func(t *testing.T) {
		tool, err := client.LoadTool("secure-tool", context.Background(), WithSecureParams(map[string]any{
	...
	t.Run("Validation failure on unused secure parameter", func(t *testing.T) {
		_, err := client.LoadTool("secure-tool", context.Background(), WithSecureParams(map[string]any{
			"api_key": "my-secret-key",
			"unused":  "extra",
		}))
		require.Error(t, err)
	...
	t.Run("Validation failure when regular param passed via WithSecureParams", func(t *testing.T) {
		_, err := client.LoadTool("secure-tool", context.Background(), WithSecureParams(map[string]any{
			"public_arg": "val",
		}))
		require.Error(t, err)
		assert.Contains(t, err.Error(), "parameter \"public_arg\" is a regular parameter; use BindParam/BindParams instead")
	})
}

func TestLoadToolset_WithSecureParams(t *testing.T) {
	mcpTools := []mcpTool{
		{
			Name: "tool1",
			InputSchema: map[string]any{
	...
	t.Run("Non-strict mode succeeds if secure params are used across toolset", func(t *testing.T) {
		tools, err := client.LoadToolset("", context.Background(), WithSecureParams(map[string]any{
			"key1": "val1",
			"key2": "val2",
		}))
		require.NoError(t, err)
	...
	t.Run("Non-strict mode fails if secure param is not used by any tool", func(t *testing.T) {
		_, err := client.LoadToolset("", context.Background(), WithSecureParams(map[string]any{
			"key1":              "val1",
			"completely_unused": "val",
		}))
		require.Error(t, err)
	...
	t.Run("Strict mode fails if any tool does not use all secure params", func(t *testing.T) {
		_, err := client.LoadToolset("", context.Background(),
			WithStrict(true),
			WithSecureParams(map[string]any{
				"key1": "val1",
			}),
		)
		require.Error(t, err)
