...
func TestToADKTool(t *testing.T) {
	...
	t.Run("Secure Parameters Isolation and Binding", func(t *testing.T) {
		...
		// Test BindSecureParam on ADK tool
		boundADKTool, err := adkTool.BindSecureParam("api_key", "secret-token-123")
		if err != nil {
			t.Fatalf("BindSecureParam failed: %v", err)
		}
		if boundADKTool.BoundSecureParams()["api_key"] != "secret-token-123" {
			t.Errorf("expected bound secure param 'secret-token-123'")
		}
	})
}
